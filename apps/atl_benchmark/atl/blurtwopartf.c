...
void blurtwopartf(float* v,int M,int N,float*output){
	v = (float *) __builtin_assume_aligned (v, 4);
	output = (float *) __builtin_assume_aligned (output, 4);
	N = N & 0xFFFFFFFC; 
	M = M & 0xFFFFFFFC; 
	float* tmp2 = (float*) aligned_alloc(4,(((1 + (N + 1) - (1)) + N + 2 - (N + 1))) * (M) * sizeof (float));
	//float* tmp2 = (float*) calloc(1,(((1 + (N + 1) - (1)) + N + 2 - (N + 1))) * (M) * sizeof (float));
